...
CMD /usr/local/openresty/nginx/sbin/nginx -p `pwd` -c nginx.conf
	...
	&&	 cp "conf/mime.types" $NGINX_FILES"/conf/"\
...
### Build the image using our Dockerfile
    ...
    sudo sudo docker build - < Dockerfile -t="edwin/openresty" .
...
### Run our newly created Docker image
    ...
    sudo docker run -t -i -p 8080:8081 -v=`pwd`:/moochine-docker -w=/moochine-docker edwin/openresty 
